...
.post( async (req, res) => {
    ...
    if(!name || !info || !image || !address) {res.json({message: 'заполните все поля!'}); return};
    if(user){
        try {
          const newHousing = await Housing.create({name, info, image, type, address, user_id: user.id}); 
          const housing = await Housing.findOne({where: {id: newHousing.id},
...
router.route('/housing/formUpdate/:id')
.get( async (req, res) => {
    try {
        const { id } = req.params;
